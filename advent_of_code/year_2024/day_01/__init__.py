...
def compute_list_distance(input_file: IO[str]) -> int:
    """
    Compute the distance between the two given lists
    :param input_file: File consisting of a lines of exactly two integers
    :return: The distance between the two lists
    :raises AssertionError: The lists are not the same length
    """
...
def compute_similarity_score(input_file: IO[str]) -> int:
    left_list, right_list = read_lists(input_file)

    return sum(e * len([x for x in right_list if x == e]) for e in left_list)
...
def read_lists(input_file):
    ...
    while 1:
        ...
        # line won't have a newline at EOF
        if not line:
            break
        if line == '\n':
            continue
        line = line.rstrip()
        parse_result = re.match(r'^\s*(-?\d+)\s+(-?\d+)\s*$', line)
        if parse_result:
            item_left, item_right = parse_result.groups()
            left_list.append(int(item_left))
            right_list.append(int(item_right))
        else:
